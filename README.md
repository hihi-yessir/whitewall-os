...
|:--------|:------|:-------------|
| **CRE** | [`access-workflow`](workflows/access-workflow/main.ts) (HTTP Trigger), [`kyc-workflow`](workflows/kyc-workflow/main.ts) (Log Trigger), [`credit-workflow`](workflows/credit-workflow/main.ts) (Log Trigger) | Workflow runtime for all 3 flows — access uses `HTTPCapability` trigger from x402 gateway, KYC and credit use `evmClient.logTrigger()` listening for `ValidationRequest` events |
