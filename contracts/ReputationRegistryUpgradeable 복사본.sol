...
contract ReputationRegistryUpgradeable is OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct Feedback {
        int128 value;          // 16 bytes (점수 음수가능)
        uint8 valueDecimals;   // 1 byte  (packed with value + isRevoked) (소수점 자리수)
        bool isRevoked;        // 1 byte  (packed with value + valueDecimals) (리뷰 지웠는지 여부)
        string tag1; //태그1
        string tag2; //태그2
    }
    ...
    struct ReputationRegistryStorage {
        ...
        //A에게 B가 준 n번째 Feedback에 답변한 모든 C들 & 그 여부
        // Track all unique responders for each feedback
        mapping(uint256 => mapping(address => mapping(uint64 => address[]))) _responders; 
        mapping(uint256 => mapping(address => mapping(uint64 => mapping(address => bool)))) _responderExists; //A에게 B가 준 n번째 Feedback에 C가 답변했는지 여부
        
        //A에게 피드백 준 모든 B들 & 그 여부
        // Track all unique clients that have given feedback for each agent
        mapping(uint256 => address[]) _clients; 
        mapping(uint256 => mapping(address => bool)) _clientExists;//A에게 피드백 준 B인지 여부
    }
    ...
    function appendResponse(
        ...
        uint64 feedbackIndex,
        string calldata responseURI,//댓글 내용 
        bytes32 responseHash
    ) external {
        require(feedbackIndex > 0, "index must be > 0");
        ...
        // Track new responder (대댓 작성자가 글쓰니, 댓쓰니도 아닌 3자일 수 있음)
    ...
    function getSummary(
        uint256 agentId,
        address[] calldata clientAddresses,//피드백 준 B들
        string calldata tag1, //필터 태그1 -> 이 태그가 있는 피드백만 집계함
        string calldata tag2
        ...
        for (uint256 i; i < clientList.length; i++) { //한 client마다
